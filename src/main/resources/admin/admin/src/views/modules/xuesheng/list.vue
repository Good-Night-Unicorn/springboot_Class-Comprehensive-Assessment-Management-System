  ...
  <div class="main-content">
    ...
    <div v-if="showFlag">
      <el-form :inline="true" :model="searchForm" class="form-content">
		...
		<el-form-item class="select" label="教师工号" prop="jiaoshigonghao">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign" 
               :header-align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('xuesheng','综合测评') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="zonghecepingCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'综合测评':'' }}</el-button>
                <el-button v-if="isAuth('xuesheng','综合测评') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="zonghecepingCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'综合测评':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('xuesheng','综合测评') && contents.tableBtnIcon == 0" type="success" size="mini" @click="zonghecepingCrossAddOrUpdateHandler(scope.row,'cross','','','')">{{ contents.tableBtnFont == 1?'综合测评':'' }}</el-button>
    ...
    <zongheceping-cross-add-or-update v-if="zonghecepingCrossAddOrUpdateFlag" :parent="this" ref="zonghecepingCrossaddOrUpdate"></zongheceping-cross-add-or-update>
...
import zonghecepingCrossAddOrUpdate from "../zongheceping/add-or-update";
export default {
  data() {
    return {
      banjiOptions: [],
      ...
      zonghecepingCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    zonghecepingCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.zonghecepingCrossAddOrUpdateFlag = true;
		...
		for (var o in obj){
		  if(o==statusColumnName && obj[o]==statusColumnValue){
			...
			this.getDataList();
		      ...
		      this.showFlag = true;
		      this.zonghecepingCrossAddOrUpdateFlag = false;
			return;
		  }
		}
	}
      this.$nextTick(() => {
      this.$refs.zonghecepingCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.$http({
